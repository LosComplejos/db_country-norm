...
class CountryConverter:
	...
	def convertAll(self,write):
		...
		if write:
			query="ALTER TABLE "+dbtable+" ADD COLUMN old_"+dbcolumnName+" char(250)"
		...
		for i in rows:
			if write:
				q2='UPDATE '+dbtable+' SET old_'+dbcolumnName+'="'+i[dbcolumnName]+'" WHERE '+dbcolumnID+'='+`i[dbcolumnID]`
			...
			if code:
				if write:
					q3='UPDATE '+dbtable+' SET '+dbcolumnName+'="'+code+'" WHERE '+dbcolumnID+'='+`i[dbcolumnID]`
